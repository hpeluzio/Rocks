...
#
# Create TOP_DIR/NUMBER_VERSION/bin directory
# 
cd "${TOP_DIR}" || exit 1
if ! test -d "${TOP_DIR}/${NUMBER_VERSION}/bin"
then
    sudo mkdir "${TOP_DIR}/${NUMBER_VERSION}/bin" || exit 1
    sudo chown -R $LUSER:$LGROUP "${TOP_DIR}/${NUMBER_VERSION}/bin" || exit 1
fi
...
./configure --prefix=${PREFIX} --enable-ltdl-install --enable-shared --enable-static || exit 1
make -j8 || exit 1
